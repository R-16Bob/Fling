...
exp_args = dict(
    ...
    learn=dict(  # 对学习的设置
        device='cuda:0' if torch.cuda.is_available() else "cpu", local_eps=8, global_eps=300, batch_size=32, optimizer=dict(name='sgd', lr=0.02, momentum=0.9)
    ),
    model=dict(  # 对模型的设置
        name='resnet8',
        input_channel=3,
    ...
    other=dict(test_freq=1, logging_path=data_dir+'/logging/cifar10_fedavg_resnet_dir_01')  # 这是存储的Log的路径
)
